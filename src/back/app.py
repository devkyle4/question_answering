...
from parsing import (
    extract_metadata,
    extract_passages,
    split_passages_into_chunks,
    save_passage_metadata,
)
...
ENV = os.environ.get(
    "ENV", "development"
)  # Default to development if no environment variable set
...
def index_documents():
    try:
        ...
        metadata_content = (
            metadata_file.read().decode("utf-8") if metadata_file else None
        )
